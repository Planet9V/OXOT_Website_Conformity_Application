/**
 * THE source-of-truth check for OJ-original corpora with applied corrigenda
 * (task 17.1): GDPR and the Data Act. Same contract and the same three
 ...
 *
 * Usage: node scripts/verify_oj_act_corpus.mjs <gdpr|data_act> [--offline]
 */
...
const ACTS = {
  gdpr: {
    ...
    bundle: "artifacts/conformity/src/data/gdprCorpusData.ts",
    marker: "2016/679",
    probes: [
      { article: 25, phrase: "Data protection by design and by default" },
      { article: 32, phrase: "Security of processing" },
      { article: 33, phrase: "not later than 72 hours after having become aware of it" },
      // Corrigendum R(02) in effect — Art 37(1)(c) reads "or", not "and":
      { article: 37, phrase: "pursuant to Article 9 or personal data relating to criminal convictions", corrected: true },
    ],
  },
  data_act: {
    dir: "docs/data_act_statutory_corpus",
    source: "source/CELEX_32023R2854_EN.html",
    bundle: "artifacts/conformity/src/data/dataActCorpusData.ts",
    marker: "2023/2854",
    probes: [
      { article: 3, phrase: "accessible to the user" },
      { article: 4, phrase: "readily available data" },
      // Corrigendum R(01) in effect — Art 48 takes point (69):
      { article: 48, phrase: "(69) Regulation (EU) 2023/2854", corrected: true },
    ],
  },
};
...
function checkVerbatimProbes() {
  ...
  for (const { article, phrase, corrected } of act.probes) {
    if (!corrected && !sourceHtml.includes(phrase)) {
