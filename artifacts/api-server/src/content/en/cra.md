...
## Choosing your CE-marking pathway

The CRA does not let you choose your conformity route freely — **Article 32** sets the procedures, and your product's classification under Articles 6–8 controls which one you may use. From **11 December 2027**, no product with digital elements can be legally placed on the EU market without a valid CE marking, affixed only after the conformity assessment is complete, the EU Declaration of Conformity is signed, and the technical documentation (Annex VII) is compiled.

```compare
Module A — Internal control (self-assessment)
- Default / Class I products only
- Manufacturer self-declares against Annex I
- Available only once a harmonised standard is fully applied
- None cited in the OJEU yet, so a third-party route is effectively required today
---
Module B+C / Module H — Notified Body
- Important Class II and Critical products — always
- Module B+C: EU-type examination + conformity to type
- Module H: full quality-assurance system audited by a Notified Body
- Or a European cybersecurity certification (EUCC) where available
```

> [!IMPORTANT]
> As of mid-2026 **zero CRA Notified Bodies have been designated**, and designation takes 12–18 months — while Class II and Critical products must be assessed by one before full application. Start the pathway analysis now: the queue forms before the standards land. The full mechanics — module-by-module process, Annex VII technical file, the Annex V Declaration of Conformity template, and the Notified-Body capacity crisis — are in the [CE Marking Pathways deep reference](/conformity/sources/cra-ce-marking-pathways.md).

## When NIS2 decides your CRA class

The CRA and [NIS2](/en/nis2) are distinct laws — NIS2 governs *operators*, the CRA governs *products* — but they interlock through one clause. Implementing Regulation (EU) 2025/2392 makes **Industrial Automation and Control Systems intended for use by NIS2 Annex I essential entities** (energy, water, transport, health, digital infrastructure, critical manufacturing) **Important Class II** — the mandatory Notified-Body tier.

```keyfacts
The legal trigger :: "intended for use by" — your declared intended use, not where a unit happens to be installed
Sold for NIS2-essential use :: Class II — mandatory Notified Body
Sold for general industry :: Class I (likely) — lighter route
Pure IT / monitoring, no control :: Class I or Default, depending on function
Same hardware, two classes :: legitimate across a portfolio — each unit still has exactly one route
Reporting interlock :: CRA vulnerability reports flow via ENISA to NIS2 CSIRTs and operators
```

The practical move for a manufacturer of dual-market products is a **Classification Decision Record per variant** — declared intended use, NIS2 Annex I exposure, and the resulting class and module — anchored on [IEC 62443](/en/iec-62443)-4-1/-4-2 evidence that serves both self-assessment and Notified-Body examination from one technical file. This is the standard output of OXOT's Phase 1 discovery. The full analysis — including the worked Siemens SIMATIC S7 example and the eight-question interlock — is in the [CRA ↔ NIS2 Class-Designation Interlock reference](/conformity/sources/cra-nis2-class-designation-interlock.pdf).

## Four journeys to CRA conformity

Where you start depends on who you are. OXOT's CRA customer journeys map the entry point, exposure and engagement sequence for each kind of organisation:

```carousel
OT product suppliers (OEMs)
PLCs, DCS components, RTUs, industrial switches, SCADA/HMI software. Multiple product lines, thin in-house CRA expertise, and Important Class II products that can't self-attest.

**Start with:** an IEC 62443 gap assessment (4–8 weeks) — it sequences the whole portfolio before the Notified-Body queue fills.
---
System integrators
Engineering, automation and DCS/SCADA integrators who carry liability where one integrated system mixes components of different security levels.

**Start with:** a 62443-2-4 conformance assessment, then SSA-certify your flagship system — it becomes the reference that wins the next tender.
---
Asset owners / critical-infrastructure operators
Energy, water, oil & gas, chemical, rail, ports, food and pharma operators with OT. NIS2 already demands "state-of-the-art" security; the CRA raises the bar on every product entering the estate.

**Start with:** a 62443-2-1 maturity read (4–8 weeks) — it scopes the multi-year CSMS programme and the board number.
```

For products with digital elements specifically, the end-to-end route — advisory, testing and Notified-Body-grade evidence in one engagement — is the [Sentyron](/en/sentyron) conformity service. The full journeys are in the [CRA Customer Journeys reference](/conformity/sources/cra-customer-journeys.pdf).
