...
/**
 * The shared verbatim reader for OJ-act corpora — since 19.1 the same
 * browsing pattern the CRA wiki established, parameterized: tabbed counts,
 * a chapter-grouped index with a filtered/total badge, search by number,
 * title or text, cite-to-clipboard, numbered paragraph chips with anchor
 * links, URL deep-links (?tab&num&q), and per-article/recital corrigenda
 * callouts read from the corpus's own provenance. Renders only what the
 * corpus carries — acts whose corrigenda are act-level notes (the
 * consolidated pipeline) simply show none per article, and the CRA-only
 * graph/commentary layers are NOT imitated here. The banner is the
